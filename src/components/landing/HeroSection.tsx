  ...
  return (
    <section className="relative min-h-[100svh] bg-gradient-hero overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-5 py-12 flex flex-col items-center text-center justify-center min-h-[100svh]">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 bg-primary/10 border border-primary/30 rounded-full px-3 py-1.5 mb-6 animate-fade-in">
          <Clock className="w-3.5 h-3.5 text-primary" />
          <span className="text-xs font-semibold text-foreground">Oferta válida somente hoje</span>
        </div>

        {/* Headline */}
        <h1 className="text-2xl font-bold text-foreground leading-tight mb-4 max-w-sm animate-fade-in-up">
          Ensinar a Palavra de Deus Nunca Foi Tão Fácil{" "}
          <span className="text-gradient-gold">— Nem Tão Profundo</span>
        </h1>

        {/* Subheadline */}
        <p className="text-sm text-muted-foreground max-w-xs mb-6 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
        ...
        <div className="grid grid-cols-2 gap-2 mb-6 animate-fade-in-up w-full max-w-xs" style={{ animationDelay: "0.2s" }}>
          {benefits.map((benefit, index) => (
            <div key={index} className="flex items-center gap-1.5 bg-card/50 rounded-lg px-2.5 py-2">
        ...
        <div className="animate-fade-in-up w-full max-w-xs" style={{ animationDelay: "0.3s" }}>
          <Button variant="cta" size="lg" className="w-full mb-3 text-sm">
